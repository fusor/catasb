...
ansible-playbook -u ${EC2_USER} --private-key ${AWS_SSH_PRIV_KEY_PATH} -i ${ANS_CODE}/library/inventory/ec2.py ${ANS_CODE}/terminate_instance.yml --extra-vars "${EXTRA_VARS}" ${extra_args} $@
